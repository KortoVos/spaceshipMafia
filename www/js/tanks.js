...
var TankTyps = [
		...
		{
			typId:2,
			image:"assets/img/cargoship.png",
			size:30,
			maxHp:300,
			speed:7
		},
		{
			typId:3,
			image:"assets/img/carrier.png",
			size:30,
			maxHp:400,
			speed:5
		},{
			typId:4,
			image:"assets/img/destroyer.png",
			size:30,
			maxHp:500,
			speed:3
		}
...
Game.prototype = {
	...
	killTank: function(tank){
		...
		if(localTank != undefined){
			//console.log(localTank.x);
			//this.$body.css('left',this.x - localTank.x - (this.w/2) + WIDTH/2 + 'px');
			//this.$body.css('top', this.y - localTank.y - (this.h/2) + HEIGHT/2 +'px');
			$('#expl' + tank.id).css('left', (tank.x - 50- localTank.x + WIDTH/2)  + 'px');
			$('#expl' + tank.id).css('top', (tank.y - 100 - localTank.y+ HEIGHT/2)  + 'px');
		}

		
		var audio = new Audio('../assets/sound/DeathFlash.flac');
		audio.play();
	...
	receiveData: function(serverData){
		...
		serverData.balls.forEach( function(serverBall){
			...
			if(b.exploding){
				b.explode();
				var audio = new Audio('../assets/sound/explosion_dull.flac');
				audio.play();
			}
		});

		//Render neutral Objects
		game.$arena.find('.neutralObjects').remove();
		serverData.objects.forEach( function(neutralO){
			var b = new neutralObject(neutralO, game.$arena);
...
function Tank(id, type, $arena, game, isLocal, x, y, hp){
	this.id = id;
	this.type = type;
	this.speed = TankTyps[type-1].speed;
	this.$arena = $arena;
	this.w = 90;
	this.h = 180;
...
Tank.prototype = {
	...
	refresh: function(){
		...
		if(localTank != undefined){
			this.$info.css('left',this.x - localTank.x  + WIDTH/2 + 'px');
			this.$info.css('top', this.y - localTank.y  + HEIGHT/2 +'px');
		}
	...
	setCannonAngle: function(){
		...
		var deltaX = this.mx - WIDTH/2;
		var deltaY = this.my - HEIGHT/2;
